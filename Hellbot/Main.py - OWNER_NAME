...
async def alive(event):
    ...
    await event.edit(f"**🔥 HellBot Userbot V5 🔥**\n\n**Owner:** `{OWNER_NAME}`\n**Version:** 5.0\n**Uptime:** `{uptime}`\n**Handler:** `{HANDLER}`\n**Abuse:** `{ABUSE}`\n**Commands:** 28+")
...
async def id_cmd(event):
    ...
    if reply:
        ...
        await event.edit(f"**Your ID:** `{event.sender_id}`\n**Chat ID:** `{event.chat_id}`")
...
async def yt_download(event):
    ...
    try:
        proc = await asyncio.create_subprocess_shell(
            f'yt-dlp -f "best[ext=mp4]" -o "video.mp4" {url}',
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        await proc.communicate()
        await client.send_file(event.chat_id, "video.mp4", caption=f"**Downloaded by {OWNER_NAME}**")
        os.remove("video.mp4")
        await event.delete()
...
async def tiktok(event):
    ...
    try:
        ...
        await proc.communicate()
        await client.send_file(event.chat_id, "tiktok.mp4", caption=f"**By {OWNER_NAME}**")
...
async def insta(event):
    ...
    try:
        ...
        await proc.communicate()
        await client.send_file(event.chat_id, "insta.mp4", caption=f"**By {OWNER_NAME}**")
        os.remove("insta.mp4")
        await event.delete()
    except: await event.edit("`Error`")
...
async def shayari(event):
    await event.edit(f"**✨ {OWNER_NAME} Ki Shayari ✨**\n\n{random.choice(SHAYARI_LIST)}")
...
async def tagall(event):
    ...
    for c in chunk:
        await event.respond(f"**{OWNER_NAME}:** {text}\n\n{' '.join(c)}")
...
async def ai_chat(event):
    ...
    try:
        ...
        if r.get("success"):
            await event.edit(f"**👑 OWNER: {OWNER_NAME}**\n**❓ Question:** {query}\n**🤖 AI Reply:** {r['success']}")
        else:
            await event.edit(f"**👑 OWNER: {OWNER_NAME}**\n`AI reply nahi mila`")
    except:
        await event.edit(f"**👑 OWNER: {OWNER_NAME}**\n`AI error`")
...
async def ban(event):
    ...
    await event.edit(f"`{OWNER_NAME} ne ban kiya:` [{reply.sender.first_name}](tg://user?id={reply.sender_id})")
...
async def unban(event):
    ...
    await event.edit(f"`{OWNER_NAME} ne unban kiya:` [{reply.sender.first_name}](tg://user?id={reply.sender_id})")
...
async def kick(event):
    ...
    await event.edit(f"`{OWNER_NAME} ne kick kiya`")
...
async def mute(event):
    ...
    await event.edit(f"`{OWNER_NAME} ne mute kiya`")
...
async def unmute(event):
    ...
    await event.edit(f"`{OWNER_NAME} ne unmute kiya`")
...
async def purge(event):
    ...
    await event.respond(f"`{OWNER_NAME} ne {len(msgs)} messages purge kiye`", delete_in=3)
...
async def sysinfo(event):
    ...
    await event.edit(f"**{OWNER_NAME}'s Bot Stats**\n**CPU:** `{cpu}%`\n**RAM:** `{ram}%`")
...
async def google(event):
    ...
    await event.edit(f"**{OWNER_NAME} searched:**\n[{query}]({url})")
...
async def calc(event):
    ...
    try:
        result = eval(expr)
        await event.edit(f"**{OWNER_NAME}'s Calc:** `{expr} = {result}`")
...
async def weather(event):
    ...
    try:
        ...
        await event.edit(f"**{OWNER_NAME} checked weather:** `{r}`")
    except: await event.edit("`Error`")
...
if ABUSE == "ON":
    ...
    async def paste(event):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://nekobin.com/api/documents", json={"content": text}) as r:
                res = await r.json()
                await event.edit(f"**{OWNER_NAME} pasted:** https://nekobin.com/{res['result']['key']}")
    ...
    async def evaluate(event):
        ...
        try:
            result = eval(cmd)
            await event.edit(f"**{OWNER_NAME}'s Eval:** `{result}`")
    ...
    async def terminal(event):
        ...
        await event.edit(f"**{OWNER_NAME}'s Terminal:**\n`{result}`")
    ...
    async def loop_cmd(event):
        ...
        for i in range(count):
            await event.respond(f"{OWNER_NAME}: {i+1}. {text}")
...
async def main():
    print(f"HellBot Userbot V5 Starting... Owner: {OWNER_NAME}")
    await client.start()
